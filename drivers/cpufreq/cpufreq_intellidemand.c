...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
        ...
        for_each_online_cpu(j) {
            ...
            if ((j_dbs_info->cur_policy != NULL) && (j_dbs_info->cur_policy->cur == j_dbs_info->cur_policy->max)) {
                if (policy->cur >= dbs_tuners_ins.optimal_freq)
                    max_load_other_cpu = dbs_tuners_ins.up_threshold_any_cpu_load;
            }
        }
...
if (num_online_cpus() > 1) {
    if (max_load_other_cpu > (dbs_tuners_ins.up_threshold_multi_core - dbs_tuners_ins.down_differential) && freq_next < dbs_tuners_ins.sync_freq)
        freq_next = dbs_tuners_ins.sync_freq;

    if (max_load_freq > (dbs_tuners_ins.up_threshold_multi_core - dbs_tuners_ins.down_differential_multi_core) * policy->cur)
...
#if CONFIG_CPUFREQ_LIMIT_MAX_FREQ
...
static void dbs_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < DBS_INPUT_EVENT_MIN_FREQ) {
		...
		*/
		if (__cpufreq_driver_target(policy, DBS_INPUT_EVENT_MIN_FREQ, CPUFREQ_RELATION_L) >= 0);
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	for_each_possible_cpu(i) {
		struct cpu_dbs_info_s *this_dbs_info = &per_cpu(id_cpu_dbs_info, i);
