	...
	if (strnlen(msm_fb_pdata->prim_panel_name, PANEL_NAME_MAX_LEN)) {
		...
		if (!strncmp((char *)msm_fb_pdata->prim_panel_name,
			...
			if (!strncmp((char *)msm_fb_pdata->prim_panel_name,
				"hdmi_msm", len))
				hdmi_prim_display = 1;
			// PL: FIXME: doesn't exist in our kernel
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	printk("PL:msm_fb_probe()\n");

	MSM_FB_DEBUG("msm_fb_probe\n");

	if ((pdev->id == 0) && (pdev->num_resources > 0)) {
	  printk("PL: msm_fb_probe, devid=0, numres > 0, ok!\n");
			...
			unsigned long arg)
{
	...
	case MSMFB_METADATA_GET:
		...
		if (!ret)
			ret = copy_to_user(argp, &mdp_metadata, sizeof(mdp_metadata));
