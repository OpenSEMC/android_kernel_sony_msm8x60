...
static int dbg_clock_dump(struct clk *clock, struct seq_file *m)
{
	int len = 0;
	u64 value = 0;
	struct clk *parent;
...
{
	int index;
	char *title_msg = "------------ DBG Clock -------------\n";

	seq_printf(m, title_msg);
	for (index = 0; index < num_msm_clocks; index++)
		dbg_clock_dump(msm_clocks[index].clk, m);
	return 0;
}
...
static int dbg_clock_status_debug_init(void)
{
	debugfs_clock_base = debugfs_create_dir("dbg_clock", NULL);
